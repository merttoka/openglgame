...
public:
	...
	Vector3D translate(const float& _x, const float& _y, const float& _z) const
	{
		return Vector3D(x+_x, y+_y, z+_z);
	}

	Vector3D rotateY(const float& angle) const
	{
		return Vector3D(x*(float)cos(angle)-z*(float)sin(angle), y, x*(float)sin(angle)+z*(float)cos(angle));
	}

	Vector3D scale(const float& _x, const float& _y, const float& _z) const
	{
		return Vector3D(x*_x,y*_y,z*_z);
	}
};